...
class _StudentSignupPageState extends State<StudentSignupPage> {
  ...
  String? _intlPhoneNumber;

  bool _isLoading = false;
  String? _error;

  // Track which verification method is active
  ...
  void initState() {
    super.initState();
    // Register static callback for deep link OTP. Defensive: only call setState when mounted.
    StudentSignupPage.onDeepLinkOtp = (verificationId) {
      if (!mounted) return;
      setState(() {
        // placeholder: store verification id or trigger UI update if needed in future
      });
    };
  }
  ...
  void dispose() {
    ...
    if (StudentSignupPage.onDeepLinkOtp != null) {
      StudentSignupPage.onDeepLinkOtp = null;
    }
    super.dispose();
  }
  ...
  void _onSmsVerification() async {
    ...
    final email = _emailController.text.trim();
    final password = _passwordController.text.trim();
    final phone = _intlPhoneNumber ?? _phoneController.text.trim();
    ...
    // Navigate directly to the full-screen phone verification route
    if (mounted) {
      context.push(
        '/verify-phone',
        extra: {
        ...
        child: Column(
          ...
          children: [
            ...
            IntlPhoneField(
              ...
              onChanged: (phone) {
                _intlPhoneNumber = phone.completeNumber;
